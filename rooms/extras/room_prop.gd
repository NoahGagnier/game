...
## Root position should sit near the trunk base. Use y_sort_offset to nudge the
## depth-sort line without moving the sprite (root shifts for Y-sort, sprite shifts back).
...
@export var shadow_color: Color = Color(0.0, 0.0, 0.0, 0.145)
...
func _apply_y_sort_offset() -> void:
	if y_sort_offset == 0.0:
		return
	var sprite := get_node_or_null("Sprite") as Sprite2D
	if sprite == null:
		return
	position.y += y_sort_offset
	sprite.position.y -= y_sort_offset
...
func _apply_sprite_layout(sprite: Sprite2D) -> void:
	...
	sprite.offset = Vector2(0.0, -sprite.texture.get_height() * 0.6)
