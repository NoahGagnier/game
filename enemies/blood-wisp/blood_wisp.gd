...
@export var hover_speed: float = 0.65
@export var shadow_ground_y: float = 12.0
@export var shadow_base_scale: Vector2 = Vector2(0.95, 0.32)
@export var shadow_color: Color = Color(0.18, 0.0, 0.0, 0.45)

@export_group("Animations")
@export var float_texture: Texture2D = preload("res://enemies/blood-wisp/assets/blood-wisp.png")
...
@export_group("Attack")
@export var projectile_scene: PackedScene = preload("res://enemies/blood-wisp/blood_wisp_projectile.tscn")
@export var projectile_damage: float = 12.0
## Min seconds between volleys.
@export var fire_interval_min: float = 0.5
## Max seconds between volleys.
...
func _physics_process(delta: float) -> void:
	...
	if _fire_timer <= 0.0 and _can_target_player():
		_fire_timer = randf_range(fire_interval_min, fire_interval_max)
		_fire_volley()
...
func _fire_volley() -> void:
	...
	for dir in [Vector2.UP, Vector2.DOWN, Vector2.LEFT, Vector2.RIGHT]:
		...
		if proj.has_method("setup"):
			proj.setup(global_position, dir, projectile_damage)
...
func _setup_animation() -> void:
	...
	if death_texture != null:
		_add_sheet_frames(
			frames, "death", death_texture,
			death_frame_count, death_frame_size, death_anim_speed, false,
		)
...
func _die() -> void:
	...
	if death_texture != null and _sprite.sprite_frames != null and _sprite.sprite_frames.has_animation("death"):
		_sprite.modulate = Color.WHITE
